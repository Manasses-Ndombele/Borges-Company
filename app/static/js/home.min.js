function handleBaseSections(entries){entries.forEach(entry=>{if(entry.isIntersecting&&entry.target.classList.contains('animated')===!1){if(entry.target.id=='results-count'){let countElements=entry.target.querySelectorAll('strong')
let actualNumber=0
setInterval(()=>{countElements.forEach((strongElement,i)=>{actualNumber=Number.parseInt(strongElement.innerHTML.split('+').join(''))
if(i==0){if(actualNumber<100){strongElement.innerHTML=`+${actualNumber + 1}`}}else if(i==1){if(actualNumber<10){strongElement.innerHTML=`+${actualNumber + 1}`}}else if(i==2){if(actualNumber<112){strongElement.innerHTML=`+${actualNumber + 1}`}}else if(i==3){if(actualNumber<3){strongElement.innerHTML=`+${actualNumber + 1}`}}})},20)}else if(entry.target.id=='history-area-img'){if(window.innerWidth<768){entry.target.classList.add('flip-in-diag-1-tr')}else if(window.innerWidth>=768){entry.target.classList.add('scale-up-ver-center')}}else if(entry.target.id=='history-container-subtitle'){entry.target.classList.add('tracking-in-expand')}else if(entry.target.id=='history-container-description'){entry.target.classList.add('text-focus-in')}else if(entry.target.id=='support-area'){entry.target.classList.add('appear-diag-enter')}
if(entry.target.classList.contains('description-container')){entry.target.classList.add('appear-diag-enter')}
entry.target.classList.add('animated')}})}
let baseSectionOptions={root:null,rootMargin:'0px',threshold:0.2}
let baseSectionObserver=new IntersectionObserver(handleBaseSections,baseSectionOptions)
let resultsContainer=document.querySelector('div#results-count')
let historyImg=document.querySelector('img#history-area-img')
let historySubtitle=document.querySelector('h2#history-container-subtitle')
let historyDescription=document.querySelector('p#history-container-description')
let supportArea=document.querySelector('div#support-area')
let descriptionContainers=document.querySelectorAll('.description-container')
let baseTargets=[resultsContainer,historyImg,historySubtitle,historyDescription,supportArea]
descriptionContainers.forEach(container=>{baseTargets.push(container)})
baseTargets.forEach(target=>{baseSectionObserver.observe(target)})
...
return words.join(' ')}
let supportForm=supportArea.querySelector('form')
