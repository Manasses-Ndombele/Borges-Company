function handleBaseSections(entries) {
    entries.forEach(entry => {
        if (entry.isIntersecting&&entry.target.classList.contains('animated')===!1) {
            if (entry.target.id=='results-count') {
                let countElements=entry.target.querySelectorAll('strong')
                let actualNumber = 0
                setInterval(() => {
                    countElements.forEach((strongElement,i) => {
                        actualNumber = Number.parseInt(strongElement.innerHTML.split('+').join(''))
                        if (i==0) {
                            if (actualNumber < 100) {
                                strongElement.innerHTML = `+${actualNumber + 1}`
                            }
                        } else if (i==1) {
                            if (actualNumber < 10) {
                                strongElement.innerHTML = `+${actualNumber + 1}`
...
baseSectionObserver.observe(resultsContainer)
