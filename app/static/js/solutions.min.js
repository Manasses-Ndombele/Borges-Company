function handleBaseSections(entries){entries.forEach(entry=>{if(entry.isIntersecting&&entry.target.classList.contains('animated')===!1){if(entry.target.id=='AI-solutions-subtitle'){entry.target.classList.add('tracking-in-expand')}else if(entry.target.id=='AI-solutions-description'){entry.target.classList.add('text-focus-in')}else if(entry.target.classList.contains('card')){if(window.innerWidth<768){entry.target.classList.add('card-rotation-enter')}else if(window.innerWidth>=768){entry.target.classList.add('scale-up-center')}}else if(entry.target.id=='support-area'){entry.target.classList.add('appear-diag-enter')}
...
let AISolutionsSubtitle=document.querySelector('h2#AI-solutions-subtitle')
let AISolutionsDescription=document.querySelector('p#AI-solutions-description')
...
let baseTargets=[AISolutionsSubtitle,AISolutionsDescription,supportArea]
descriptionContainers.forEach(container=>{baseTargets.push(container)})
cards.forEach(card=>{baseTargets.push(card)})
